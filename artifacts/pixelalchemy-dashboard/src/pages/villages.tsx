...
import { ArrowUpRight, Cpu, Search, SlidersHorizontal, Users } from "lucide-react";
...
export default function Villages() {
  ...
  const queryParams = useMemo<GetVillagesParams>(
    () => ({
      search: search || undefined,
      zone: zone === "All zones" ? undefined : (zone as GetVillagesParams["zone"]),
      priority: priority === "All priorities" ? undefined : (priority as GetVillagesParams["priority"]),
    }),
    [search, zone, priority]
  );
  const query = useGetVillages(queryParams);

  if (query.isLoading) return <DashboardShell><LoadingState label="Reading village priority register" /></DashboardShell>;
  ...
  return (
    <DashboardShell>
      ...
      <section className="border border-[#d7d1c5] bg-[#fbf9f3]">
        ...
        {villages.length === 0 ? (
          <EmptyState title="No villages match those filters" description="Try a wider search or reset the filters to see all villages in the register." />
        ) : (
            ...
            <table className="w-full min-w-[860px] border-collapse text-left">
              <thead>
                <tr className="border-b border-[#e3ded4] bg-[#f2eee4] font-mono text-[10px] uppercase tracking-[.12em] text-[#60717c]">
                  ...
                  <th className="px-3 py-3 font-normal">ML Landslide Prob</th>
                  <th className="px-3 py-3 font-normal">Est. Demand</th>
                  <th className="px-3 py-3 font-normal">Priority Score</th>
